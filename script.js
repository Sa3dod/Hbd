...
function initScene3() {
  ...
  const drop = (sel, dur) => {
    // tier falls from above, lands with squash & bounce + stand shudder
    tl.fromTo(sel,
      { opacity: 1, y: -300, scaleY: 1.06, scaleX: 0.96, transformOrigin: 'bottom center' },
      { y: 0, duration: dur, ease: 'power2.in' }
    )
    .to(sel, { scaleY: 0.82, scaleX: 1.1, duration: 0.09, ease: 'power1.out' })
    .to(sel, { scaleY: 1, scaleX: 1, duration: 0.34, ease: 'elastic.out(1.1, 0.45)' })
    .fromTo('#cakeSvg', { y: 0 }, { y: 2.5, duration: 0.05, yoyo: true, repeat: 1 }, '<');
  };

  const drip = (sel) => {
    // cream melts downward
    tl.fromTo(sel,
      { opacity: 0, scaleY: 0.2, transformOrigin: 'top center' },
      { opacity: 1, scaleY: 1, duration: 0.5, ease: 'back.out(1.4)' }, '-=0.1');
  };
  ...
  drop('#cakeBottom', 0.42); drip('#bottomCream');
  drop('#cakeMiddle', 0.38); drip('#middleCream');
  drop('#cakeTop',    0.34); drip('#topCream');

  tl
    // daisies twirl in one by one
    .fromTo('#cakeDaisies', { opacity: 1 }, { duration: 0.01 })
    .fromTo('#cakeDaisies > g',
      { opacity: 0, scale: 0, rotation: -160, transformOrigin: 'center center' },
      { opacity: 1, scale: 1, rotation: 0, duration: 0.45, ease: 'back.out(2.4)', stagger: 0.09 }
    )
    // sparkles twinkle in
    .fromTo('#cakeSparkles', { opacity: 1 }, { duration: 0.01 })
    .fromTo('#cakeSparkles text',
      { opacity: 0, scale: 0.3, transformOrigin: 'center center' },
      { opacity: 1, scale: 1, duration: 0.3, ease: 'back.out(2)', stagger: 0.07 }, '-=0.3'
    )
    // candles pop up...
    .fromTo('#cakeCandles3 .s3-candle',
      { opacity: 0, scaleY: 0, transformOrigin: 'bottom center' },
      { opacity: 1, scaleY: 1, duration: 0.3, ease: 'back.out(2)', stagger: 0.1 }
    )
    // ...then flames IGNITE with a flash
    .fromTo('#cakeCandles3 .s3-flamewrap',
      { opacity: 0, scale: 0, transformOrigin: 'center bottom' },
      { opacity: 1, scale: 1, duration: 0.4, ease: 'elastic.out(1.4, 0.5)', stagger: 0.1 }, '-=0.25'
    )
    // final flourish: whole cake breathes + confetti puff
    .to('#cakeSvg', { scale: 1.025, transformOrigin: 'center bottom', duration: 0.18, yoyo: true, repeat: 1, ease: 'power1.inOut' })
    .call(() => {
      confetti({ particleCount: 26, spread: 75, startVelocity: 24, origin: { y: 0.5 },
        colors: ['#4fb3e8', '#ef6fa4', '#b487dd', '#f2c14e', '#ffffff'] });
      const footer = document.getElementById('scene3Footer');
      footer.classList.remove('hidden');
      gsap.fromTo(footer, { y: 16, opacity: 0 }, { y: 0, opacity: 1, duration: 0.5 });
    });
...
function initScene7() {
  ...
  // #4 birthday-day live countdown to midnight
  const cd = document.getElementById('bdayCountdown');
  if (cd) {
    const now0 = new Date();
    if (now0.getMonth() === 6 && now0.getDate() === 23) {
      cd.classList.remove('hidden');
      const mid = new Date(now0.getFullYear(), 6, 24, 0, 0, 0);
      const tick = () => {
        const ms = mid - new Date();
        if (ms <= 0) { cd.innerHTML = 'hope it was unforgettable 🌙'; clearInterval(cdIv); return; }
        const h = String(Math.floor(ms / 3600000)).padStart(2, '0');
        const m = String(Math.floor(ms % 3600000 / 60000)).padStart(2, '0');
        const s = String(Math.floor(ms % 60000 / 1000)).padStart(2, '0');
        cd.innerHTML = 'your birthday ends in <strong>' + h + ':' + m + ':' + s + '</strong> — make it count ⏳';
      };
      tick();
      const cdIv = setInterval(tick, 1000);
    }
  }

  // #6 hidden message: typing dots -> funny message -> seen receipt
  const secret = document.getElementById('secretMsg');
  if (secret && !secret.dataset.wired) {
    secret.dataset.wired = '1';
    setTimeout(() => {
      gsap.to(secret, { opacity: 1, duration: 0.8 });          // bubble appears, dots bouncing
      setTimeout(() => {
        secret.querySelector('.secret-dots').style.display = 'none';
        const t = secret.querySelector('.secret-text');
        t.classList.remove('hidden');
        haptic(12);
        gsap.fromTo(secret.querySelector('.secret-bubble'),
          { scale: 0.92 }, { scale: 1, duration: 0.35, ease: 'back.out(2.5)' });
        setTimeout(() => {
          const seen = document.getElementById('secretSeen');
          seen.classList.remove('hidden');
          gsap.fromTo(seen, { opacity: 0 }, { opacity: 1, duration: 0.6 });
        }, 2200);
      }, 1900);
    }, 8000);
  }

  // #1 The End overlay + replay
  ...
  if (endBtn && !endBtn.dataset.wired) {
    ...
    endBtn.addEventListener('click', () => {
      ...
      confetti({ particleCount: 50, spread: 90, origin: { y: 0.4 },
        colors: ['#fff8e7', '#f2c14e', '#b487dd', '#4fb3e8', '#ef6fa4'] });
      gsap.fromTo('.finale-inner', { y: 24, opacity: 0 }, { y: 0, opacity: 1, duration: 0.8, ease: 'power2.out', delay: 0.2 });
    });
...
============================================================ */
const NOTES = [
  { text: 'Doğum günün kutlu olsun Kraliçe', meaning: 'Happy birthday, Queen' },
